...
import (
	...
	"errors"
	"fmt"
	networkname2 "github.com/erigontech/erigon-lib/chain/networkname"
...
func RecoverSendersBatch(ethereum *eth.Ethereum, signer *types.Signer, start, end uint64) error {
	...
	for nr := start; nr <= end; nr++ {
		header := rawdb.ReadHeaderByNumber(tx, nr)
		if header == nil {
			log.Info("Recovery: header not found", "blockNum", nr)
			continue
		}
		block := rawdb.ReadBlock(tx, header.Hash(), nr)
		if block == nil {
			log.Info("Recovery: block not found", "blockNum", nr)
			continue
		}
...
func RecoverRegenesis(ethereum *eth.Ethereum) error {
	...
	case networkname2.OPMainnetChainName:
		genesisHeader.Root = libcommon.HexToHash("0xeddb4c1786789419153a27c4c80ff44a2226b6eda04f7e22ce5bae892ea568eb")
		targetGenesisHash = libcommon.HexToHash("0x7ca38a1916c42007829c55e69d3e9a73265554b586a499015373241b8a3fa48b")
	default:
