...
func (e *EngineBlockDownloader) download(hashToDownload libcommon.Hash, heightToDownload uint64, downloaderTip libcommon.Hash, requestId int, block *types.Block) {
	/* Start download process*/
	// First we schedule the headers download process
	if !e.scheduleHeadersDownload(requestId, hashToDownload, heightToDownload, downloaderTip) {
...
func (e *EngineBlockDownloader) StartDownloading(requestId int, hashToDownload libcommon.Hash, heightToDownload uint64, downloaderTip libcommon.Hash, blockTip *types.Block) bool {
	e.lock.Lock()
	defer e.lock.Unlock()
	...
	go e.download(hashToDownload, heightToDownload, downloaderTip, requestId, blockTip)
	return true
}
