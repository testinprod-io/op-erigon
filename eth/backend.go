...
func New(ctx context.Context, stack *node.Node, config *ethconfig.Config, logger log.Logger) (*Ethereum, error) {
	...
	if err := backend.chainDB.Update(context.Background(), func(tx kv.RwTx) error {
		...
		chainConfig, genesis, genesisErr = core.WriteGenesisBlock(tx, genesisSpec, config.OverrideCancunTime, config.OverrideShanghaiTime, config.OverrideOptimismCanyonTime, config.OverrideOptimismEcotoneTime, config.OverrideOptimismFjordTime, config.OverrideOptimismGraniteTime, config.OverrideOptimismHoloceneTime, config.OverrideOptimismIsthmusTime, config.OverrideOptimismJovianTime, config.OverridePragueTime, tmpdir, logger)
	...
	if chainConfig.IsOptimism() {
		...
		if chainConfig.IsthmusTime == nil {
			log.Warn("Optimism IsthmusTime has not been set")
		}
		if chainConfig.JovianTime == nil {
			log.Warn("Optimism JovianTime has not been set")
		}
	}
	...
	mining := stagedsync.New(
		...
		stagedsync.MiningStages(backend.sentryCtx,
			...
			stagedsync.StageTrieCfg(backend.chainDB, false, true, true, tmpdir, blockReader, nil, config.HistoryV3, backend.agg, chainConfig),
