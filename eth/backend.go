...
func (s *Ethereum) Init(stack *node.Node, config *ethconfig.Config) error {
	...
	apiList := jsonrpc.APIList(chainKv, borDb, ethRpcClient, txPoolRpcClient, miningRpcClient, ff, stateCache, blockReader, s.agg, httpRpcCfg, s.engine, s.seqRPCService, s.historicalRPCService, s.logger)
	go func() {
