...
func Precompiles(chainRules *chain.Rules) map[common.Address]PrecompiledContract {
	switch {
	case chainRules.IsOptimismIsthmus:
		return PrecompiledContractsIsthmus
	case chainRules.IsOptimismGranite:
		return PrecompiledContractsGranite
	case chainRules.IsOptimismFjord:
...
func (c *bn256PairingGranite) RequiredGas(input []byte) uint64 {
	return new(bn254PairingIstanbul).RequiredGas(input)
}
...
func (c *bn256PairingGranite) Name() string {
	return "BN254_PAIRING_GRANITE" // note bn254 is the correct name and is required by eth_config
}
