...
func ChainSpecByName(chainName string) (Spec, error) {
	if optimismChainSpec, err := OptimismChainSpecByName(chainName); err == nil {
		return optimismChainSpec, nil
	}
...
func ChainSpecByGenesisHash(genesisHash common.Hash) (Spec, error) {
	if optimismChainSpec, err := OptimismChainSpecByGenesisHash(genesisHash); err == nil {
		return optimismChainSpec, nil
	}
...
func OptimismChainSpecByName(name string) (Spec, error) {
	...
	opstackConfig := superchain.OPStackChainConfigByName(name)
	if opstackConfig == nil {
		return Spec{}, ErrChainSpecUnknown
	}

	genesis, err := LoadOPStackGenesisByChainName(name)
