...
func ValidateOptimismExtraData(fc ForkChecker, time uint64, extraData []byte) error {
	if fc.IsJovian(time) {
		...
		return fmt.Errorf("extraData must be empty before Holocene")
	}
	return nil
}
...
func EncodeOptimismExtraData(fc ForkChecker, time uint64, denominator, elasticity uint32, minBaseFee *uint64) []byte {
	if fc.IsJovian(time) {
		if minBaseFee == nil {
...
func EncodeJovianExtraData(denom, elasticity uint32, minBaseFee uint64) []byte {
	r := make([]byte, 17)
	r[0] = JovianExtraDataVersionByte
	binary.BigEndian.PutUint32(r[1:5], denom)
	binary.BigEndian.PutUint32(r[5:9], elasticity)
